...
from model.faster_rcnn_2fc.vgg16 import vgg16
from model.faster_rcnn_2fc.resnet_twostage import resnet
import pdb
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        help='res101.yml',
                        default='cfgs/personpart_2s/res50_multiscale.yml', type=str)
...
if __name__ == '__main__':
    ...
    if args.dataset == "pascal_voc":
        ...
        args.imdbval_name = "personpart_train"
    ...
    load_name = os.path.join(input_dir,
                             'faster_rcnn_{}_{}_{}.pth'.format(args.checksession, args.checkepoch, cfg.RESUME_CHECKPOINT))
    ...
    for i in range(1):
            ...
            if args.test_mode == 'all':
                if j == 1:
                    ...
                    if inds.numel() > 0 and inds_hf.numel() > 0:
                        # cls_scores_hf = scores_hf[:, j - 1][inds_hf]
                        # _, order_hf = torch.sort(cls_scores_hf, 0, True)
                        # if cfg.CLASS_AGNOSTIC:
                        #     cls_boxes_hf = pred_boxes_hf[inds_hf, :]
                        # else:
                        #     cls_boxes_hf = pred_boxes_hf[inds_hf][:, (j - 1) * 4:((j - 1) + 1) * 4]
                        #
                        # cls_dets_hf = torch.cat((cls_boxes_hf, cls_scores_hf.unsqueeze(1)), 1)
                        # cls_dets_hf = cls_dets_hf[order_hf]
                        # keep_hf = nms(cls_dets_hf, 0.2)
                        # cls_dets_hf = cls_dets_hf[keep_hf.view(-1).long()]
                        #
                        # cls_scores = scores[:, j][inds]
                        # _, order = torch.sort(cls_scores, 0, True)
                        # if cfg.CLASS_AGNOSTIC:
                        ...
                        # cls_dets = cls_dets[keep.view(-1).long()]
                        #
                        # cls_dets_all = torch.cat((cls_dets, cls_dets_hf), 0)
                        # cls_scores_all = torch.cat((scores[[keep.view(-1).long()]][:, j], scores_hf[keep_hf.view(-1).long()][:, (j-1)]), 0)
                        # _, order_all = torch.sort(cls_scores_all, 0, True)
                        #
                        # cls_dets_all = cls_dets_all[order_all]
                        # keep_all = nms(cls_dets_all, cfg.TEST.NMS)
                        # cls_dets_all = cls_dets_all[keep_all.view(-1).long()]
                        # if vis:
                        #     im2show = vis_detections(im2show, imdb.classes[j], cls_dets_all.cpu().numpy(), vis_thresh)
                        # all_boxes[j][i] = cls_dets_all.cpu().numpy()
        ...
        sys.stdout.flush()
        if args.test_mode == 'all':
            output_dir = input_dir + '/output_all_' + str(args.checkepoch)
        if args.test_mode == '1s':
            output_dir = input_dir + '/output_1s_' + str(args.checkepoch)
        if args.test_mode == '2s':
            output_dir = input_dir + '/output_2s_' + str(args.checkepoch)
