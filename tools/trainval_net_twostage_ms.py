...
from model.faster_rcnn_ms.vgg16 import vgg16
from model.faster_rcnn_ms.resnet_ms import resnet
...
def parse_args():
  ...
  parser.add_argument('--cfg', dest='cfg_file',
                      help='res101.yml',
                      default='cfgs/personpart_2s/res101_421.yml', type=str)
...
if __name__ == '__main__':
  ...
  for epoch in range(args.start_epoch, cfg.TRAIN.EPOCHS + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      # im2show_ms = vis_rois(im2show_ms, boxes_handface.cpu().numpy(), rois_label_hf.data.cpu().numpy(), person_flag=0)
      # cv2.imwrite('./debug/' + str(step) + '_ms.png', im2show_ms)
      ...
      # im2show_ms = vis_rois(im2show_ms, pred_boxes_hf.view(-1, pred_boxes_hf.size(2)).cpu().numpy(), rois_label_hf.data.cpu().numpy(), person_flag=0)
      # cv2.imwrite('./debug_rcnn/' + str(step) + '_ms.png', im2show_ms)
      # ######################## debug ########################
