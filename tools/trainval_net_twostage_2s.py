...
from model.utils.net_utils import weights_normal_init, save_net, load_net, save_checkpoint, clip_gradient
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        help='res101.yml',
                        default='cfgs/personpart_2s/res101.yml', type=str)
...
def adjust_learning_rate(args, epoch, optimizer, decay=0.1):
...
if __name__ == '__main__':
    ...
    dataset = roibatchLoader(roidb, ratio_list, ratio_index, cfg.TRAIN.IMG_BATCH_SIZE, imdb.num_classes, training=True)
    ...
    base_params = get_trainable_param(fasterRCNN, 'RCNN_base')
    stage1_params = get_trainable_param(fasterRCNN, 'RCNN_rpn,RCNN_top,RCNN_bbox_pred,RCNN_cls_score')
    stage2_params = get_trainable_param(fasterRCNN, 'RCNN_rpn_handface,RCNN_top_handface,RCNN_bbox_pred_handface,RCNN_cls_score_handface')
    ...
    for epoch in range(args.start_epoch, cfg.TRAIN.EPOCHS + 1):
        ...
        for param_group in optimizer.param_groups: print("lr after adjusting: ", param_group['lr'])
        ...
        if args.mGPUs:
            save_name = os.path.join(output_dir,
                                     'faster_rcnn_{}_{}_{}.pth'.format(args.session, epoch, cfg.CHECKPOINT))
            save_checkpoint({
