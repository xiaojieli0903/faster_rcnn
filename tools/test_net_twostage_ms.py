...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        help='res101.yml',
                        default='cfgs/personpart_2s/res101_strengthen.yml', type=str)
...
if __name__ == '__main__':
    ...
    for i in range(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            if args.test_mode == 'all':
                if j == 1:
                    ...
                    inds_hf = torch.nonzero(scores_hf[:, (j-1)] > hf_thresh_ms).view(-1)
                    # if there is det
        ...
        if args.test_mode == '2s':
            output_dir = input_dir + '/' + cfg.CHECKPOINT + '/output_ms'
