    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            if class_name == 'person':
                ...
                cv2.rectangle(im, bbox[0:2], bbox[2:4], (255, 182, 193), 2)
                cv2.putText(im, '%s: %.3f' % (class_name, score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
                            1.0, (0, 0, 255), thickness=1)
...
def vis_rois(im, rois, rois_label,person_flag):
    """Visual debugging of detections."""
    for i in range(rois.shape[0]):
        bbox = tuple(int(np.round(x)) for x in rois[i, :4])
        if person_flag:
            if rois_label[i] > 0:
                if rois_label[i] == 2:
                    cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 100, 100), 2)
                elif rois_label[i] == 3:
                    cv2.rectangle(im, bbox[0:2], bbox[2:4], (204, 0, 0), 2)
                else:
                    cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 204, 0), 2)
                cv2.putText(im, '%.3f' % (rois_label[i]), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
                            1.0, (0, 0, 255), thickness=1)
            else:
                cv2.rectangle(im, bbox[0:2], bbox[2:4], (55, 55, 55), 2)
        else:
            if rois_label[i] > 0:
                if rois_label[i] == 1:
                    cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 100, 100), 2)
...
def vis_gts(im, gts, num):
    """Visual debugging of detections."""
    for i in range(num):
        bbox = tuple(int(np.round(x)) for x in gts[0, i, :4])
        cv2.rectangle(im, bbox[0:2], bbox[2:4], (255, 182, 193), 2)
    return im
