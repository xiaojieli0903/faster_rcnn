...
from model.rpn_2fc_41.rpn import _RPN
from model.roi_pooling.modules.roi_pool import _RoIPooling
from model.roi_crop.modules.roi_crop import _RoICrop
from model.roi_align.modules.roi_align import RoIAlignAvg
from model.rpn_2fc_41.proposal_target_layer_cascade import _ProposalTargetLayer

from model.rpn_2fc_41.bbox_transform import clip_boxes, clip_boxes_batch
from model.rpn_2fc_41.bbox_transform import bbox_transform_inv, bbox_transform_batch
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.RCNN_rpn_handface = _RPN(512, person_flag=0)
        ...
        self.RCNN_roi_pool_person = _RoIPooling(cfg.POOLING_SIZE_PERSON, cfg.POOLING_SIZE_PERSON, 1.0 / 8.0)  # 24
        self.RCNN_roi_align_person = RoIAlignAvg(cfg.POOLING_SIZE_PERSON, cfg.POOLING_SIZE_PERSON, 1.0 / 8.0)

        self.RCNN_roi_pool_hf = _RoIPooling(cfg.POOLING_SIZE_HF, cfg.POOLING_SIZE_HF, 1.0 / 8.0)  # 7
        self.RCNN_roi_align_hf = RoIAlignAvg(cfg.POOLING_SIZE_HF, cfg.POOLING_SIZE_HF, 1.0 / 8.0)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        base_feat_layer1 = self.RCNN_layer1(im_data)
        base_feat_layer2 = self.RCNN_layer2(base_feat_layer1)
        base_feat_layer3 = self.RCNN_layer3(base_feat_layer2)
        base_feat_layer4 = self.RCNN_top(base_feat_layer3)
        # ((2L, 256L, 225L, 150L), (2L, 512L, 113L, 75L), (2L, 1024L, 57L, 38L), (2L, 2048L, 57L, 38L))
        new_feat = self.RCNN_conv_new(base_feat_layer4) # batch_size * 512 * h * w
        new_feat_hf = self.RCNN_conv_new_hf(base_feat_layer2)  # batch_size * 512 * h * w

        # -------------------1.Person_rpn: feed base feature map tp RPN to obtain rois-------------------#
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat_layer3, im_info, gt_boxes, num_boxes, person_flag=1)
        ...
        if cfg.POOLING_MODE == 'align':
            pooled_feat = self.RCNN_roi_align(new_feat, rois.view(-1, 5))
        elif cfg.POOLING_MODE == 'pool':
            pooled_feat = self.RCNN_roi_pool(new_feat, rois.view(-1, 5))
        # feed pooled features to top model
        # compute bbox offset
        pooled_feat = pooled_feat.view(pooled_feat.size(0), -1)
        pooled_feat = self.RCNN_2fc(pooled_feat)
        ...
        # do roi pooling based on predicted rois
        if cfg.POOLING_MODE == 'align':
            pooled_feat_person = self.RCNN_roi_align_person(new_feat_hf, person_rois.view(-1, 5))
        elif cfg.POOLING_MODE == 'pool':
            pooled_feat_person = self.RCNN_roi_pool_person(new_feat_hf, person_rois.view(-1, 5))
        ...
        # feed pooled features to top model
        pooled_feat_handface = self.RCNN_2fc_hf(pooled_feat_handface.view(pooled_feat_handface.size(0), -1))
    ...
    def _init_weights(self):
        ...
        normal_init(self.RCNN_bbox_pred_handface, 0, 0.01, cfg.TRAIN.TRUNCATED)

        weights_init(self.RCNN_conv_new, 0, 0.01, cfg.TRAIN.TRUNCATED)
        weights_init(self.RCNN_conv_new_hf, 0, 0.01, cfg.TRAIN.TRUNCATED)
        weights_init(self.RCNN_2fc, 0, 0.01, cfg.TRAIN.TRUNCATED)
        weights_init(self.RCNN_2fc_hf, 0, 0.01, cfg.TRAIN.TRUNCATED)
                    ...
                    for k in range(gt_boxes.size(1)):
                        if gt_boxes[i][k][4] == 2 or gt_boxes[i][k][4] == 3:
                            ...
                            if (w * h) / gt_area >= cfg.RATIO:
                                ...
                                scale_x = cfg.POOLING_SIZE_PERSON * 8 / (
                                            person_rois.data[i, j, 3] - person_rois.data[i, j, 1])
                                scale_y = cfg.POOLING_SIZE_PERSON * 8 / (
