...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        if cfg.POOLING_MODE == 'align':
            pooled_feat_handface = self.RCNN_roi_align_hf(base_feat, rois_handface.view(-1, 5))
        elif cfg.POOLING_MODE == 'pool':
            pooled_feat_handface = self.RCNN_roi_pool_hf(base_feat, rois_handface.view(-1, 5))
        # feed pooled features to top model
    ...
    def _init_weights(self):
        ...
        weights_init(self.RCNN_top_handface, 0, 0.01, cfg.TRAIN.TRUNCATED)
