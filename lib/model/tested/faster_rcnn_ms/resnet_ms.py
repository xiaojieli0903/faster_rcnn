...
from model.faster_rcnn_ms.faster_rcnn_ms import _fasterRCNN
...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_top = nn.Sequential(
      nn.Conv2d(2048, 1024, kernel_size=cfg.POOLING_SIZE, stride=cfg.POOLING_SIZE, padding=0),
      nn.ReLU(True),
      nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0),
      nn.ReLU(True)
      )
    self.RCNN_top_hf = nn.Sequential(
      nn.Conv2d(256, 1024, kernel_size=cfg.POOLING_SIZE_HF, stride=cfg.POOLING_SIZE_HF, padding=0),
      nn.ReLU(True),
      nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0),
      nn.ReLU(True)
    )
